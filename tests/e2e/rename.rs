...
fn rename_moves_file_and_rewrites_backlinks() {
    ...
    let a_out = repo.zdb()
        .args(["create", "--title", "Linker A", "--body",
    ...
    repo.zdb()
        .args(["create", "--title", "Linker C", "--body",
...
fn rename_no_backlinks() {
    ...
    let out = repo.zdb()
        .args(["create", "--title", "Lonely", "--body", "No one links here."])
        .output().unwrap();
